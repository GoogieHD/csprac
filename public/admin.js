...
socket.on("selectCaptains", () => {
  if (currentPlayerPool.length === 0) {
    console.warn("No players in currentPlayerPool yet. Waiting...");
    return;
  }
  displayArea.innerHTML = `
      ...
      <ul id="captainSelectList" class="space-y-2">
        ${currentPlayerPool
          .map(
            (p) => `
          <li>
            <button data-id="${p.id}" class="px-4 py-2 bg-yellow-600 hover:bg-yellow-700 rounded text-white w-full">
              ${p.name}
            </button>
          </li>`
          )
...
socket.on("draftComplete", ({ teamA, teamB }) => {
  ...
  document.getElementById("startMapVotingBtn").onclick = () => {
    const rawMapPool = document.getElementById("mapPoolInput").value;
    const mapPool = rawMapPool
      .split(",")
      .map((m) => m.trim())
      .filter((m) => m.length > 0);
    if (mapPool.length < 2) {
    ...
    <p class="mb-2">
      ${
        isCaptain
          ...
          : "Waiting for the current captain to make a pick..."
      }
    </p>
...
socket.on("mapChosen", ({ finalMap, teamA, teamB }) => {
  ...
  target.innerHTML = `
    ...
    <p class="text-center text-gray-300 mt-6">Prepare to load into <strong>${finalMap}</strong>.</p>
  `;
});
